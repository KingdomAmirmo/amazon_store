    ...
    public function copanDiscount(Request $request)
    {
        ...
        if ($copan != null)
        {
            if ($copan->user_id != null)
            {
                ...
                if ($copan == null)
                {
                    return redirect()->back()->with('swal-error', 'کد تخفیف نامعتبر است');
                }
            }
        ...
        if ($request->payment_type == 1) {
            ...
            foreach ($cartItems as $cartItem) {
                OrderItem::create([
                    ...
                    'amazing_sale_discount_amount' => empty($cartItem->product->activeAmazingSale()) ? 0 : ($cartItem->cartItemProductPrice() * ($cartItem->product->activeAmazingSale()->percentage / 100)) * ($cartItem->number),
                    'number' => $cartItem->number,
                    'final_product_price' => ($cartItem->product->price) - (empty($cartItem->product->activeAmazingSale()) ? 0 : $cartItem->cartItemProductPrice() * ($cartItem->product->activeAmazingSale()->percentage / 100)),
                    'final_total_price' => ($cartItem->product->price) - (empty($cartItem->product->activeAmazingSale()) ? 0 : $cartItem->cartItemProductPrice() * ($cartItem->product->activeAmazingSale()->percentage / 100)) * ($cartItem->number),
