<aside id="sidebar" class="sidebar col-md-3">
    ...
    <form action="{{ route('customer.products', ['category' => request()->category ? request()->category->id : null]) }}" method="get">
        {{-- for send sort type while submit price filter--}}
                ...
                @foreach($brands as $brand)
                    <section class="form-check sidebar-brand-item">
                        ...
                        <input class="form-check-input" type="checkbox" name="brands[]"
                               @if(request()->brands && in_array($brand->id, request('brands')))
                                   checked
                               @endif
    ...
    <div class="mb-3">
        <a href="{{ route('customer.products') }}" class="btn btn-warning rounded-3">حذف فیلتر ها</a>
    </div>
</aside>
