                                ...
                                <form id="cart_items" method="post" class="content-wrapper bg-white p-3 rounded-2">
                                    ...
                                    @foreach($cartItems as $cartItem)
                                        ...
                                        <section class="cart-item d-md-flex py-3">
                                            <section class="cart-img align-self-start flex-shrink-1"><img src="{{ asset($cartItem->product->image['indexArray']['medium']) }}" alt=""></section>
                                                <section class="align-self-start w-100">
                                                    ...
                                                    <section>
                                                        <section class="cart-product-number d-inline-block ">
                                                            ...
                                                            <input class="number" data-product-price="{{ $cartItem->cartItemProductPrice() }}"
                                                                   data-product-discount="{{ $cartItem->cartItemProductDiscount() }}" type="number" name="number[{{ $cartItem->id }}]" min="1" max="5" step="1" value="{{ $cartItem->number }}" readonly="readonly">
                                                            <button class="cart-number cart-number-up" type="button">+</button>
                                                        </section>
                                                        <a class="text-decoration-none ms-4 cart-delete" href="{{ route('customer.sales-process.remove-from-cart', $cartItem) }}"><i class="fa fa-trash-alt"></i> حذف از سبد خرید</a>
                                                    </section>
                                                </section>
                                    ...
                                    <section class="item">
                                        <section class="lazyload-item-wrapper">
                                            <section class="product">
                                                ...
                                                <a class="product-link" href="{{ route('customer.market.product', $relatedProduct->slog) }}">
                                                    <section class="product-image">
                                                        <img class="" src="{{ asset($relatedProduct->image['indexArray']['medium']) }}" alt="{{ asset($relatedProduct->image['indexArray']['medium']) . '-' . $key }}">
                                                    </section>
    ...
    <script>
        ...
        function bill() {
            ...
            $('.number').each(function (){
                ...
                var product_discount_price = parseFloat($(this).data('product-discount'));
                var number = parseFloat($(this).val());

                total_product_price += product_price * number;
                total_discount += product_discount_price * number;
            });
